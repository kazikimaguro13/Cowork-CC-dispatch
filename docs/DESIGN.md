...
## 9. v2 — 夜間自律保守ループ（実装完了 / version 0.18.0）

> ステータス: 全3フェーズ実装完了（spec_013〜029、version 0.18.0、テスト564、サブコマンド12）。実装の記録と実走で発覚した欠陥は §9.8 を参照。
...
§9 の設計を17本の spec（spec_013〜029）に割り、1本ずつ Claude Code に dispatch して実装した（CCD 自身の bridge 機構を使ったドッグフーディング）。version 0.4.0 → 0.18.0、テスト 206 → 564、サブコマンド 6 → 12。
...
**実走で初めて炙り出された3つの欠陥** — 紙の設計レビューでは見抜けず、実際に動かしたことでのみ表面化した: (1) 発見が実リポジトリを汚染（迷子コミット `impl spec_100`。§9.4 の隔離制約 ＋ spec_014 で修正）/ (2) ミューテーション撃破率0%（PEP 660 editable install の import hook ＋ mutmut パーサの取り違え。spec_019 でカナリア検証つきで修正、撃破率81%へ）/ (3) 正しい修正を書いたループが偽HALT（翻訳の制約文言を修正係が誤読。spec_026 で修正）。安全設計（隔離・ガード・カナリア・偽陽性優先）を機能より先に組んでいたため、いずれも「実害で済み、気づかぬまま悪化することはなかった」。
