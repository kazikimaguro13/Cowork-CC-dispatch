...
## 9. v2 — 夜間自律保守ループ（実装完了 / version 0.20.0）

> ステータス: 全3フェーズ + Phase 2.5（複数施策 sweep 運用 + 沈黙失敗の構造修正 + mutmut ネスト構造互換性）実装完了（spec_013〜032、version 0.20.0、テスト622、サブコマンド12）。実装の記録と実走で発覚した欠陥は §9.8 を参照。
...
§9 の設計を 20 本の spec（spec_013〜032）に割り、1 本ずつ Claude Code に dispatch して実装した（CCD 自身の bridge 機構を使ったドッグフーディング）。version 0.4.0 → 0.20.0、テスト 206 → 622、サブコマンド 6 → 12。
