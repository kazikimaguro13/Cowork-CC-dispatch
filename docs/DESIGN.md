...
## 9. v2 — 夜間自律保守ループ（実装完了 / version 0.19.1）

> ステータス: 全3フェーズ + Phase 2.5（複数施策 sweep 運用 + 沈黙失敗の構造修正）実装完了（spec_013〜031、version 0.19.1、テスト606、サブコマンド12）。実装の記録と実走で発覚した欠陥は §9.8 を参照。
...
§9 の設計を 19 本の spec（spec_013〜031）に割り、1 本ずつ Claude Code に dispatch して実装した（CCD 自身の bridge 機構を使ったドッグフーディング）。version 0.4.0 → 0.19.1、テスト 206 → 606、サブコマンド 6 → 12。
...
- **Phase 2.5 沈黙失敗の構造修正**（spec_030〜031）— Phase 2.5 の初回実走 sweep で発覚した「**任意 repo に向けたときの沈黙失敗**」2 種を構造修正。

**実走で初めて炙り出された欠陥 5 件 + 残課題 1 件** — 紙の設計レビューでは見抜けず、実際に動かしたことでのみ表面化:
...
6. **mutmut が axis のネスト構造（`backend/src/...`）で 0 mutants を返す**（Phase 2.5 #5 実走、原因切り分け中）── HALT として可視化されているため誤情報は出ない、構造修正は spec_032 候補（任意・後回し）

安全設計（隔離・ガード・カナリア・偽陽性優先）を機能より先に組んでいたため、5 件すべて「実害で済み、気づかぬまま悪化することはなかった」。6 件目（mutmut 互換性）は spec_030/031 の防護網がそのまま捕まえて HALT として可視化している ── これがまさに「正直な計測」原則の効能。
